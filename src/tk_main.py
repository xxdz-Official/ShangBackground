...
def _cancel_slide_timer_locked():
    global slide_timer, slide_generation
    slide_generation += 1
    if slide_timer:
        try:
...
def _schedule_slide_timer_locked():
    global slide_timer, slide_generation
    slide_generation += 1
    current_generation = slide_generation
    delay_ms = int(config["slide_seconds"] * 1000)
    if root is not None:
        slide_timer = root.after(delay_ms, lambda g=current_generation: slide_next(g))
    else:
        slide_timer = threading.Timer(config["slide_seconds"], lambda: slide_next(current_generation))
        slide_timer.daemon = True
        slide_timer.start()


def slide_next(schedule_generation=None):
    global slide_timer, slide_enabled
    with slide_timer_lock:
        if not slide_enabled:
            return
        if schedule_generation is not None and schedule_generation != slide_generation:
            log("忽略过期的幻灯片定时任务")
            return
        slide_timer = None
...
def reset_slide_timer():
    ...
    with slide_timer_lock:
        ...
        if current not in slide_images:
            return
        _cancel_slide_timer_locked()
        _schedule_slide_timer_locked()


def start_slideshow():
    global slide_images, slide_enabled
    with slide_timer_lock:
        if config["mode"] != "幻灯片放映":
            return False
        folder = config["slide_folder"]
    ...
    try:
        if os.path.exists(MACOS_MENU_COMMAND_FILE):
            ...
            if command == "show":
                ...
                subprocess.Popen([sys.executable, os.path.abspath(__file__), "--quick-settings"])
            elif command == "switch-mode":
                subprocess.Popen([sys.executable, os.path.abspath(__file__), "--switch-mode"])
            elif command == "open-folder":
                subprocess.Popen([sys.executable, os.path.abspath(__file__), "--open-wallpaper-folder"])
            elif command == "about":
                subprocess.Popen([sys.executable, os.path.abspath(__file__), "--about"])
    except Exception as e:
...
def get_macos_service_path():
    return os.path.expanduser("~/Library/Services/Set As Wallpaper.workflow")


def get_macos_action_service_path(action):
    safe_name = f"ShangBackground {action}.workflow"
    return os.path.expanduser(os.path.join("~/Library/Services", safe_name))
...
def create_macos_set_wallpaper_service():
    service_dir = get_macos_service_path()
    target_script = os.path.abspath(sys.argv[0])
    python_executable = shlex.quote(os.path.abspath(sys.executable))
    script = (
        'for f in "$@"; do\n'
        '  if [ -f "$f" ]; then\n'
        f'    {python_executable} {shlex.quote(target_script)} --set-wallpaper "$f"\n'
        '  fi\n'
        'done\n'
    )
    ...
    log(f"macOS 服务已安装: {service_dir}")


def remove_macos_set_wallpaper_service():
    service_dir = get_macos_service_path()
    if os.path.exists(service_dir):
        try:
            shutil.rmtree(service_dir)
            log(f"macOS 服务已移除: {service_dir}")
        except Exception as e:
            log(f"移除 macOS 服务失败: {e}")
...
def create_macos_action_service(action, title, arg):
    service_dir = get_macos_action_service_path(action)
    python_executable = shlex.quote(os.path.abspath(sys.executable))
    ...
    log(f"macOS 系统右键服务已安装: {service_dir}")


def remove_macos_action_service(action):
    service_dir = get_macos_action_service_path(action)
    if os.path.exists(service_dir):
        try:
            shutil.rmtree(service_dir)
            log(f"macOS 系统右键服务已移除: {service_dir}")
        except Exception as e:
            log(f"移除 macOS 系统右键服务失败: {e}")


def sync_macos_context_services():
    ...
    for action, title, arg, config_key, requires_slide in MACOS_CONTEXT_ACTIONS:
        ...
        if enabled:
            ...
            remove_macos_action_service(action)


def register_context():
    windows_supported = IS_WINDOWS and winreg is not None
    if IS_MACOS:
        log("macOS 将使用 Finder 系统服务菜单集成右键条目")
        try:
            if config.get("ctx_set_wallpaper", False):
                create_macos_set_wallpaper_service()
            else:
                remove_macos_set_wallpaper_service()
        except Exception as e:
            log(f"macOS Finder 右键服务注册失败: {e}")
        try:
            sync_macos_context_services()
        except Exception as e:
            log(f"macOS 系统右键服务处理失败: {e}")
        return
...
def toggle_ctx_set_wallpaper():
    global ctx_file_wallpaper_var
    config["ctx_set_wallpaper"] = ctx_file_wallpaper_var.get()
    save_config()
    register_context()
...
def update_wallpaper_preview():
    ...
    for i, img_file in enumerate(images[:3]):
        ...
        try:
            ...
            frame = tk.Frame(preview_images_frame, relief="flat", borderwidth=0, highlightthickness=0, bg=UI_PANEL)
...
class InitSettingsDialog:
    ...
    def __init__(self, parent):
        ...
        self.dialog.configure(bg=UI_PANEL)
    ...
    def create_widgets(self):
        # 主框架
        main_frame = tk.Frame(self.dialog, bg=UI_PANEL)
        main_frame.pack(fill="both", expand=True)

        # 左侧说明区域
        left_frame = tk.Frame(main_frame, bg=UI_ACCENT_SOFT, width=140)
        left_frame.pack(side="left", fill="y")
        left_frame.pack_propagate(False)

        title_label = tk.Label(left_frame, text="初始化设置", font=(FONT_FAMILY, 14, "bold"), bg=UI_ACCENT_SOFT, fg=UI_TEXT)
        title_label.pack(pady=(40, 10))

        desc_label = tk.Label(left_frame, text="清除配置数据\n恢复软件初始状态", font=(FONT_FAMILY, 9), bg=UI_ACCENT_SOFT,
                              fg=UI_MUTED, justify="center")
        desc_label.pack(pady=(10, 0))

        # 右侧内容区域
        right_frame = tk.Frame(main_frame, bg=UI_PANEL)
        ...
        desc_label = tk.Label(right_frame, text="选择要清除的数据类型", font=("Segoe UI", 9),
                              bg=UI_PANEL, fg=UI_MUTED)
        desc_label.pack(anchor="w", pady=(0, 15))

        # 选项列表框架
        list_frame = tk.Frame(right_frame, bg=UI_PANEL, relief="flat", bd=1, highlightthickness=1,
                              highlightbackground=UI_BORDER_SOFT)
        list_frame.pack(fill="both", expand=True, pady=(0, 15))

        # Canvas 和 Scrollbar 实现滚动
        canvas = tk.Canvas(list_frame, bg=UI_PANEL, highlightthickness=0)
        scrollbar = ttk.Scrollbar(list_frame, orient="vertical", command=canvas.yview)
        scrollable_frame = tk.Frame(canvas, bg=UI_PANEL)
        ...
        select_all_frame = tk.Frame(scrollable_frame, bg=UI_PANEL)
        ...
        # 分隔线
        sep_line = tk.Frame(scrollable_frame, bg=UI_BORDER_SOFT, height=1)
        ...
        for option in self.clear_options_list:
            key = option["key"]
            # 选项容器
            item_frame = tk.Frame(scrollable_frame, bg=UI_PANEL)
        ...
        # 按钮区域
        btn_frame = tk.Frame(right_frame, bg=UI_PANEL)
...
class GlobalShortcutDialog:
    ...
    def create_widgets(self):
        ...
        title_label = tk.Label(left_frame, text="全局快捷键", font=(FONT_FAMILY, 14, "bold"), bg=UI_ACCENT_SOFT, fg=UI_TEXT)
        title_label.pack(pady=(40, 10))

        desc_label = tk.Label(left_frame, text="设置全局快捷键\n快速切换壁纸", font=(FONT_FAMILY, 9), bg=UI_ACCENT_SOFT,
                              fg=UI_MUTED, justify="center")
        desc_label.pack(pady=(10, 0))

        # 右侧内容区域
        right_main = tk.Frame(main_frame, bg=UI_PANEL)
...
class TransitionSettingsDialog:
    ...
    def create_widgets(self):
        ...
        title_label = tk.Label(left_frame, text="过渡动画", font=(FONT_FAMILY, 14, "bold"), bg=UI_ACCENT_SOFT, fg=UI_TEXT)
        title_label.pack(pady=(40, 10))

        desc_label = tk.Label(left_frame,
                              text="设置壁纸切换时的\n动画效果不过无法调用原生\n壁纸切换动画\n所以此动画为自制甚至可以\n达到原生比支持的动画效果\n？！",
                              font=(FONT_FAMILY, 8), bg=UI_ACCENT_SOFT,
                              fg=UI_MUTED, justify="center")
        desc_label.pack(pady=(10, 0))
        ...
        # 效果选择
        effect_row = tk.Frame(right_frame, bg=UI_PANEL)
        effect_row.pack(fill="x", pady=(0, 20))

        tk.Label(effect_row, text="转场原理", font=(FONT_FAMILY, 10), fg=UI_TEXT, bg=UI_PANEL, width=8,
        ...
        self.smooth_effect_row = tk.Frame(right_frame, bg=UI_PANEL)
        tk.Label(self.smooth_effect_row, text="转场效果", font=(FONT_FAMILY, 10), fg=UI_TEXT, bg=UI_PANEL, width=8,
                 anchor="w").pack(side="left")
        ...
        self.slide_direction_row = tk.Frame(right_frame, bg=UI_PANEL)
        tk.Label(self.slide_direction_row, text="动画方向", font=(FONT_FAMILY, 10), fg=UI_TEXT, bg=UI_PANEL, width=8,
                 anchor="w").pack(side="left")
        ...
        # 持续时间
        duration_row = tk.Frame(right_frame, bg=UI_PANEL)
        duration_row.pack(fill="x", pady=(0, 20))

        tk.Label(duration_row, text="持续时间", font=(FONT_FAMILY, 10), fg=UI_TEXT, bg=UI_PANEL, width=8,
                 anchor="w").pack(side="left")
        ...
        tk.Label(duration_row, text="秒", font=(FONT_FAMILY, 10), fg=UI_MUTED, bg=UI_PANEL).pack(side="left")
        ...
        # 再放置帧数行
        self.frame_row = tk.Frame(right_frame, bg=UI_PANEL)

        tk.Label(self.frame_row, text="帧数", font=(FONT_FAMILY, 10), fg=UI_TEXT, bg=UI_PANEL, width=8,
                 anchor="w").pack(side="left")
        ...
        tk.Label(self.frame_row, text="帧", font=(FONT_FAMILY, 10), fg=UI_MUTED, bg=UI_PANEL).pack(side="left")
        ...
        # 弹性空间，将按钮推到底部
        spacer = tk.Frame(right_frame, bg=UI_PANEL, height=1)
...
def create_color_button(parent, color, row, col, command):
    ...
    def on_enter(e): btn.config(highlightbackground=UI_ACCENT, highlightthickness=2)

    def on_leave(e): btn.config(highlightbackground=UI_BORDER, highlightthickness=1)
...
def create_gradient_button(parent, color1, color2, row, col, command):
    ...
    def on_enter(e):
        btn.config(highlightbackground=UI_ACCENT, highlightthickness=2)

    def on_leave(e):
        btn.config(highlightbackground=UI_BORDER, highlightthickness=1)
...
def main():
    ...
    main_container = RoundedFrame(root, radius=24, bg=UI_PANEL, padding=16)
    main_container.pack(fill="both", expand=True, padx=20, pady=16)
    main_content = main_container.content
    ...
    preview_frame = RoundedFrame(main_content, radius=18, bg=UI_PANEL, padding=12)
    preview_frame.configure(width=400)
    preview_frame.pack(side="left", fill="both", expand=True, padx=(0, 18))
    ...
    canvas.pack(pady=18)
    util_frame = ttk.Frame(preview_content, style="Panel.TFrame")
    ...
    def open_tray_settings():
        ...
        # 设置样式
        style = ttk.Style()
        style.configure("Modern.TFrame", background=UI_PANEL)
        style.configure("Modern.TLabel", background=UI_PANEL, font=(FONT_FAMILY, 10))
        style.configure("Modern.TButton", font=(FONT_FAMILY, 9), padding=5)

        # 主容器（使用Frame而不是ttk.Frame以支持侧边栏）
        main_frame = tk.Frame(win, bg=UI_PANEL)
        main_frame.pack(fill="both", expand=True)
        ...
        title_label = tk.Label(left_frame, text="托盘功能", font=(FONT_FAMILY, 14, "bold"), bg=UI_ACCENT_SOFT, fg=UI_TEXT)
        title_label.pack(pady=(40, 10))

        desc_label = tk.Label(left_frame, text="设置系统托盘图标的\n菜单项和行为\n\nOwO 为已启用\nXwX 为已禁用",
                              font=(FONT_FAMILY, 9), bg=UI_ACCENT_SOFT,
                              fg=UI_MUTED, justify="center")
        desc_label.pack(pady=(10, 0))
        ...
        tip_label = ttk.Label(title_bar, text=">w<  拖拽排序 & 勾选启用/禁用",
                              font=(FONT_FAMILY, 8), foreground=UI_MUTED)
        ...
        # 配置高亮标签
        tree.tag_configure("drag_highlight", background=UI_ACCENT_SOFT, foreground=UI_TEXT)
        tree.tag_configure("normal", background=UI_PANEL, foreground=UI_TEXT)
    ...
    settings_frame = RoundedFrame(main_content, radius=18, bg=UI_PANEL, padding=12)
    settings_frame.configure(width=500)
    settings_frame.pack(side="right", fill="both", expand=True)
    settings_frame.pack_propagate(False)
    settings_content = settings_frame.content
    mode_frame = ttk.Frame(settings_content, style="Panel.TFrame")
    ...
    dynamic_frame = ttk.Frame(settings_content, style="Panel.TFrame")
    ...
    def open_custom_time_dialog():
        ...
        main_frame = tk.Frame(dialog, bg=UI_PANEL, padx=20, pady=20)
        main_frame.pack(fill="both", expand=True)

        tk.Label(main_frame, text="输入时间数值:", font=(FONT_FAMILY, 10), bg=UI_PANEL).pack(anchor="w")
        value_frame = tk.Frame(main_frame, bg=UI_PANEL)
        ...
        error_label = tk.Label(main_frame, text="", fg=UI_DANGER, bg=UI_PANEL, font=(FONT_FAMILY, 8))
        ...
        btn_frame = tk.Frame(main_frame, bg=UI_PANEL)
    ...
    extra_frame = ttk.Frame(settings_content, style="Panel.TFrame")
    ...
    def update_ctx_checkbutton_texts():
        ...
        if IS_MACOS:
            chk_prev.config(text="添加【Finder右键服务 → 上一张壁纸】")
            chk_next.config(text="添加【Finder右键服务 → 下一张壁纸】")
            chk_random.config(text="添加【Finder右键服务 → 随机壁纸】")
            chk_jump.config(text="添加【Finder右键服务 → 跳转到壁纸】")
            chk_file_wallpaper.config(text="添加【Finder文件右键 → 设为壁纸】")
            chk_personalize.config(text="添加【Finder右键服务 → 显示主界面】")
            return
        ...
        chk_file_wallpaper.config(text="添加【Finder右键 → 设为壁纸】")
    ...
    ctx_file_wallpaper_var = tk.BooleanVar(value=config.get("ctx_set_wallpaper", False))
    global chk_file_wallpaper
    chk_file_wallpaper = ttk.Checkbutton(extra_frame, text="", variable=ctx_file_wallpaper_var,
                                         command=toggle_ctx_set_wallpaper)
    chk_file_wallpaper.pack(anchor="w", pady=0)
    ...
    if is_first_time:
        ...
        if not config.get("auto_start_prompt_shown", False):
            ...
            # 图片和标题行（图片在左，文字在右）
            top_row = tk.Frame(main_frame, bg=UI_PANEL)
            ...
            if os.path.exists(hello_img_path):
                try:
                    ...
                    hello_label = tk.Label(top_row, image=hello_photo, bg=UI_PANEL)
            ...
            desc_label = tk.Label(top_row, text="您是否想要开机自启动本工具？",
                                  font=(FONT_FAMILY, 15, "bold"), bg=UI_PANEL, fg=UI_TEXT)
            desc_label.pack(side="left")

            # 文本框（紧贴上方，无外边距）
            info_text = tk.Text(main_frame, height=2, wrap="word", font=(FONT_FAMILY, 12),
                                bg=UI_SURFACE, fg=UI_TEXT, relief="flat", borderwidth=1,
            ...
            # 按钮容器 - 用于右对齐
            button_container = tk.Frame(btn_frame, bg=UI_PANEL)
            button_container.pack(side="right")

            # 好哒 按钮
            btn_yes = tk.Button(button_container, text="好哒", command=on_yes,
                                bg=UI_ACCENT, fg=UI_PANEL, font=(FONT_FAMILY, 9),
                                relief="flat", padx=15, pady=5, cursor="hand2")
            btn_yes.pack(side="left", padx=(0, 10))

            # 不，并不再提示 按钮
            btn_no = tk.Button(button_container, text="不，并不再提示", command=on_no,
                               bg=UI_SURFACE, fg=UI_TEXT, font=(FONT_FAMILY, 9),
                               relief="flat", padx=15, pady=5, cursor="hand2")
            btn_no.pack(side="left")

            # 按钮悬停效果
            def on_enter_yes(e):
                btn_yes.config(bg=UI_ACCENT_DARK)

            def on_leave_yes(e):
                btn_yes.config(bg=UI_ACCENT)

            def on_enter_no(e):
                btn_no.config(bg=UI_BORDER_SOFT)

            def on_leave_no(e):
                btn_no.config(bg=UI_SURFACE)
    ...
    def show_update_prompt():
        ...
        title_label = tk.Label(main_frame, text=f"✨ 发现新版本 v{remote_version}.0！",
                               font=(FONT_FAMILY, 14, "bold"), bg=UI_PANEL, fg=UI_TEXT)
        ...
        def on_ignore():
            ...
            if hasattr(root, 'version_label') and root.version_label is not None:
                root.version_label.config(text="[版本] v1.0 忽略了更新", fg=UI_SUCCESS, cursor="")
        ...
        btn_ignore = tk.Button(button_container, text="忽略本次更新并不再提醒", command=on_ignore,
                               bg=UI_PANEL, fg=UI_MUTED, font=(FONT_FAMILY, 9),
                               relief="solid", borderwidth=1, padx=15, pady=5, cursor="hand2")
        btn_ignore.pack(side="left")

        # 按钮悬停效果
        def on_enter_update(e):
            btn_update.config(bg=UI_ACCENT_DARK)

        def on_leave_update(e):
            btn_update.config(bg=UI_ACCENT)

        def on_enter_later(e):
            btn_later.config(bg=UI_BORDER_SOFT)

        def on_leave_later(e):
            btn_later.config(bg=UI_SURFACE)

        def on_enter_ignore(e):
            btn_ignore.config(bg=UI_BG)

        def on_leave_ignore(e):
            btn_ignore.config(bg=UI_PANEL)
    ...
    def _create_update_window():
        ...
        notes_text = tk.Text(notes_frame, wrap="word", font=("Segoe UI", 10),
                             bg=UI_PANEL, relief="flat", borderwidth=0, height=5)
        ...
        def ignore_this_version():
            ...
            update_win.destroy()
            if hasattr(root, 'version_label') and root.version_label is not None:
                root.version_label.config(text="[版本] v1.0 忽略了更新", fg=UI_MUTED, cursor="")
                root.version_label.unbind("<Button-1>")
                root.version_label.bind("<Button-1>", lambda e: messagebox.showinfo("我の版本信息  OwO",
                                                                                    f"当前版本: v1.0\n最新版本: {remote_version}\n{remote_release_notes}"))
            messagebox.showinfo("忽略！", f"已忽略版本v {remote_version} .0 的更新,这个选择真明智（？）。。", parent=root)
    ...
    def update_version_label():
        ...
        if hasattr(root, 'version_label') and root.version_label is not None:
            try:
                ignored_version = config.get("ignored_version", "")
                if check_failed:
                    root.version_label.config(text="[版本] v1.0 检查失败", fg=UI_SUCCESS, cursor="")
                    root.version_label.unbind("<Button-1>")
                    ...
                    root.version_label.config(text="[版本] v1.0 忽略了更新", fg=UI_SUCCESS, cursor="hand2")
                    root.version_label.unbind("<Button-1>")
                    root.version_label.bind("<Button-1>", lambda e: show_update_window())
                elif show_update_flag:
                    root.version_label.config(text="[版本] v1.0 有新版本!", fg=UI_DANGER, cursor="hand2")
                    root.version_label.unbind("<Button-1>")
                    root.version_label.bind("<Button-1>", lambda e: show_update_window())
                else:
                    root.version_label.config(text="[版本] v1.0 已是最新版本", fg=UI_SUCCESS, cursor="")
                    root.version_label.unbind("<Button-1>")
                    root.version_label.bind("<Button-1>", lambda e: messagebox.showinfo("我の版本信息  OwO",
        ...
        try:
            ...
            def show_about_window():
                ...
                def create_stats_and_fetch():
                    ...
                    sub_window.configure(bg=UI_ACCENT_SOFT)
                    # 设置窗口图标
                    icon_path = os.path.join(BASE_DIR, "img", "LOGO.ico")
                    if os.path.exists(icon_path):
                        try:
                    ...
                    # 主框架，水平排列
                    main_frame = tk.Frame(sub_window, bg=UI_PANEL)
                    main_frame.pack(fill='both', expand=True)

                    # 左侧彩色矩形（固定宽度142，高度随窗口拉伸）
                    left_frame = tk.Frame(main_frame, bg=UI_ACCENT_SOFT, width=50)
                    left_frame.pack(side='left', fill='y')
                    # 防止宽度被压缩
                    left_frame.pack_propagate(False)

                    # 右侧内容框架
                    right_frame = tk.Frame(main_frame, bg=UI_PANEL, padx=15, pady=15)
                    right_frame.pack(side='left', fill='both', expand=True)
                    ...
                    for text, url in links:
                        ...
                        if "】" in text:
                            ...
                            label_frame = tk.Frame(right_frame, bg=UI_PANEL)
                            label_frame.pack(anchor="w", pady=3)
                            pink_label = tk.Label(label_frame, text=bracket_part, font=(FONT_FAMILY, 10), fg=UI_MUTED,
                                                  bg=UI_PANEL, cursor="hand2")
                            pink_label.pack(side="left")
                            blue_label = tk.Label(label_frame, text=rest_part, font=(FONT_FAMILY, 10), fg=UI_ACCENT,
                                                  bg=UI_PANEL, cursor="hand2")
                            ...
                            link_label = tk.Label(right_frame, text=text, font=(FONT_FAMILY, 10), fg=UI_ACCENT,
                                                  bg=UI_PANEL, cursor="hand2", anchor="w")
