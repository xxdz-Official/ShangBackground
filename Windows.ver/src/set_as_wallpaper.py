...
def _user_data_dir():
    if IS_MACOS:
        ...
        # Windows: 优先用 APPDATA，避免写入只读的打包目录
        if IS_FROZEN:
...
# 日志路径：打包时写入 APPDATA/ShangBackground，源码运行时写入桌面
    ...
    for proc in psutil.process_iter(['pid', 'cmdline', 'name']):
        try:
            ...
            if IS_FROZEN and IS_WINDOWS:
                ...
                # 源码运行：匹配 main.py / main.pyw 或 pythonw.exe
                matched = (
...
def start_main_program():
    ...
    if IS_FROZEN:
        # sys.executable 在 PyInstaller 打包后即为 ShangBackground.exe
