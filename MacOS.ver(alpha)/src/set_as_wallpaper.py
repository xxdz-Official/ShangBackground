...
try:
    from app_config import IS_WINDOWS, IS_MACOS, IS_LINUX
except Exception:
...
LEGACY_CONFIG_PATH = os.path.join(BASE_DIR, "shezhi.json")
...
def set_wallpaper(path):
    ...
    if IS_MACOS:
        ...
        subprocess.run(["osascript", "-e", f'tell application "System Events" to tell every desktop to set picture to POSIX file "{escaped}"'], check=True, timeout=10)
        return
    if IS_LINUX:
        abs_path = os.path.abspath(path)
        uri = "file://" + abs_path
        for key in ("picture-uri-dark", "picture-uri"):
            subprocess.run(["gsettings", "set", "org.gnome.desktop.background", key, uri], timeout=10)
        return
    ...
    for proc in psutil.process_iter(['pid', 'cmdline', 'name']):
        try:
            ...
            if IS_FROZEN and IS_WINDOWS:
                ...
                matched = (
                    'main.py' in cmdline_str or 'main.pyw' in cmdline_str
                    or (IS_WINDOWS and 'pythonw.exe' in proc_name)
                )
            if matched:
...
def start_main_program():
    """启动主程序。打包后运行 exe 本身；源码模式使用 pythonw.exe。"""
    kwargs = {}
    if IS_WINDOWS:
