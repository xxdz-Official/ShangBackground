...
def set_wallpaper_direct(path, operation_name="系统", skip_history=False):
    ...
    try:
        ...
        try:
            ...
            winreg.SetValueEx(key, "WallpaperStyle", 0, winreg.REG_SZ, str(style_map[fit_mode]))
            winreg.SetValueEx(key, "TileWallpaper", 0, winreg.REG_SZ, "1" if fit_mode == "平铺" else "0")
            winreg.CloseKey(key)
        except Exception as e:
            log("设置适应模式失败: " + str(e))
...
def set_fit_mode(mode):
    try:
        key = winreg.OpenKey(winreg.HKEY_CURRENT_USER, r"Control Panel\Desktop", 0, winreg.KEY_WRITE)
        winreg.SetValueEx(key, "WallpaperStyle", 0, winreg.REG_SZ, str(style_map[mode]))
        winreg.SetValueEx(key, "TileWallpaper", 0, winreg.REG_SZ, "1" if mode == "平铺" else "0")
        winreg.CloseKey(key)
        current = config.get("current_wallpaper")
...
def update_preview(img_path):
    ...
    try:
        ...
        if not img_path or not os.path.isfile(img_path):
            canvas.delete("all")
            canvas.create_text(190, 120, text="暂无预览", font=("微软雅黑", 12), fill="#666")
            show_overlay()
...
class InitSettingsDialog:
    ...
    def create_widgets(self):
        ...
        title_label = tk.Label(left_frame, text="初始化设置", font=("微软雅黑", 14, "bold"), bg="#9BFFF2", fg="#2c3e50")
        title_label.pack(pady=(40, 10))

        desc_label = tk.Label(left_frame, text="清除配置数据\n恢复软件初始状态", font=("微软雅黑", 9), bg="#9BFFF2",
...
class GlobalShortcutDialog:
    ...
    def create_widgets(self):
        ...
        title_label = tk.Label(left_frame, text="全局快捷键", font=("微软雅黑", 14, "bold"), bg="#9BFFF2", fg="#2c3e50")
        title_label.pack(pady=(40, 10))

        desc_label = tk.Label(left_frame, text="设置全局快捷键\n快速切换壁纸", font=("微软雅黑", 9), bg="#9BFFF2",
                              fg="#7f8c8d", justify="center")
        desc_label.pack(pady=(10, 0))
        ...
        self.listbox = tk.Listbox(left_list_frame, height=6, width=18, font=("微软雅黑", 10), relief="sunken", bd=1)
        ...
        # 当前功能标签
        self.current_func_label = ttk.Label(right_frame, text="当前功能：下一张壁纸", font=("微软雅黑", 10, "bold"))
    ...
    def start_recording(self):
        ...
        ttk.Label(record_win, text="请按下快捷键组合...\n(按ESC取消)", font=("微软雅黑", 11)).pack(expand=True, pady=20)
...
class TransitionSettingsDialog:
    ...
    def create_widgets(self):
        ...
        title_label = tk.Label(left_frame, text="过渡动画", font=("微软雅黑", 14, "bold"), bg="#9BFFF2", fg="#2c3e50")
        title_label.pack(pady=(40, 10))

        desc_label = tk.Label(left_frame,
                              text="设置壁纸切换时的\n动画效果不过无法调用原生\n壁纸切换动画\n所以此动画为自制甚至可以\n达到原生比支持的动画效果\n？！",
                              font=("微软雅黑", 8), bg="#9BFFF2",
                              fg="#7f8c8d", justify="center")
        desc_label.pack(pady=(10, 0))
        ...
        tk.Label(effect_row, text="转场原理", font=("微软雅黑", 10), fg="#34495e", bg="#ffffff", width=8,
        ...
        tk.Label(self.smooth_effect_row, text="转场效果", font=("微软雅黑", 10), fg="#34495e", bg="#ffffff", width=8,
                 anchor="w").pack(side="left")
        ...
        tk.Label(self.slide_direction_row, text="动画方向", font=("微软雅黑", 10), fg="#34495e", bg="#ffffff", width=8,
                 anchor="w").pack(side="left")
        ...
        tk.Label(duration_row, text="持续时间", font=("微软雅黑", 10), fg="#34495e", bg="#ffffff", width=8,
                 anchor="w").pack(side="left")
        ...
        duration_spinbox = ttk.Spinbox(duration_row, from_=0.1, to=10.0, increment=0.1, textvariable=self.duration_var,
                                       width=8, font=("微软雅黑", 10), command=on_duration_spinbox_change)
        duration_spinbox.pack(side="left", padx=(0, 10))

        tk.Label(duration_row, text="秒", font=("微软雅黑", 10), fg="#7f8c8d", bg="#ffffff").pack(side="left")
        ...
        tk.Label(self.frame_row, text="帧数", font=("微软雅黑", 10), fg="#34495e", bg="#ffffff", width=8,
                 anchor="w").pack(side="left")
        ...
        frame_spinbox = ttk.Spinbox(self.frame_row, from_=4, to=30, textvariable=self.frame_var, width=8,
                                    font=("微软雅黑", 10), command=on_frame_spinbox_change)
        frame_spinbox.pack(side="left", padx=(0, 10))

        tk.Label(self.frame_row, text="帧", font=("微软雅黑", 10), fg="#7f8c8d", bg="#ffffff").pack(side="left")
...
def handle_command_line():
    ...
    if args.previous:
        ...
        hwnd_existing = ctypes.windll.user32.FindWindowW(WND_CLASS_NAME, None)
        if hwnd_existing:
            log("找到已有窗口，尝试激活")
        ...
        try:
            ...
            app = WallpaperSidebar(sidebar_root, folder, current, None)  # log_path 传入None，日志已禁用
...
def main():
    ...
    global ctx_prev_var, ctx_next_var, ctx_random_var, ctx_personalize_var, ctx_jump_var, ctx_file_wallpaper_var, wallpaper_monitor_running
    ...
    flag_file = os.path.join(os.environ.get('TEMP', ''), 'WallpaperHideFlag.tmp')
    ...
    canvas = tk.Canvas(preview_frame, bg="#f9f9f9", width=380, height=240, highlightthickness=1,
                       highlightbackground="#dddddd", highlightcolor="#dddddd")
    canvas.pack(pady=20)
    util_frame = ttk.Frame(preview_frame)
    util_frame.pack(fill="x", pady=(20, 0))
    ttk.Label(util_frame, text="实用设置", font=("微软雅黑", 12, "bold")).pack(anchor="w", pady=(0, 3))
    ...
    def open_tray_settings():
        ...
        style.configure("Modern.TLabel", background="#ffffff", font=("微软雅黑", 10))
        style.configure("Modern.TButton", font=("微软雅黑", 9), padding=5)
        ...
        title_label = tk.Label(left_frame, text="托盘功能", font=("微软雅黑", 14, "bold"), bg="#9BFFF2", fg="#2c3e50")
        title_label.pack(pady=(40, 10))

        desc_label = tk.Label(left_frame, text="设置系统托盘图标的\n菜单项和行为\n\nOwO 为已启用\nXwX 为已禁用",
                              font=("微软雅黑", 9), bg="#9BFFF2",
                              fg="#7f8c8d", justify="center")
        desc_label.pack(pady=(10, 0))
        ...
        # 标题
        title_label = tk.Label(content_frame, text="托盘图标设置", font=("微软雅黑", 16, "bold"),
                               bg="#ffffff", fg="#2c3e50")
        ...
        ttk.Label(click_frame, text="单击托盘图标", font=("微软雅黑", 11, "bold"),
        ...
        click_combo = ttk.Combobox(combo_container, values=["下一张壁纸"],
                                   state="disabled", width=18, font=("微软雅黑", 9))
        click_combo.set("下一张壁纸")
        click_combo.pack(side="left")

        dev_label = tk.Label(combo_container, text="<-- 单击动作功能开发中捏...QwQ", font=("微软雅黑", 8),
                             fg="#e74c3c", bg="#ffffff")
        ...
        ttk.Label(title_bar, text="右键菜单项", font=("微软雅黑", 11, "bold"),
    ...
    ttk.Label(mode_frame, text="背景模式", font=("微软雅黑", 12)).pack(anchor="w", pady=(0, 6))
    ...
    ttk.Label(slide_frame, text="幻灯片设置", font=("微软雅黑", 10, "bold")).pack(anchor="w", pady=(0, 10))
    ...
    def open_custom_time_dialog():
        ...
        tk.Label(main_frame, text="输入时间数值:", font=("微软雅黑", 10), bg="#ffffff").pack(anchor="w")
        value_frame = tk.Frame(main_frame, bg="#ffffff")
        value_frame.pack(fill="x", pady=(5, 10))

        time_value = tk.StringVar(value="1")
        value_entry = tk.Entry(value_frame, textvariable=time_value, width=10, font=("微软雅黑", 10))
        ...
        error_label = tk.Label(main_frame, text="", fg="red", bg="#ffffff", font=("微软雅黑", 8))
    ...
    ttk.Label(single_frame, text="图片设置", font=("微软雅黑", 10, "bold")).pack(anchor="w", pady=(0, 10))
    ...
    def browse_single():
        f = filedialog.askopenfilename(filetypes=[("图片", "*.jpg;*.jpeg;*.png;*.bmp")])
        if f:
    ...
    ttk.Label(gradient_frame, text="渐变设置", font=("微软雅黑", 10, "bold")).pack(anchor="w", pady=(0, 10))
    ...
    ttk.Label(solid_frame, text="纯色设置", font=("微软雅黑", 10, "bold")).pack(anchor="w", pady=(0, 10))
    ...
    ttk.Label(extra_frame, text="右键菜单设置", font=("微软雅黑", 12, "bold")).pack(anchor="w", pady=(0, 0))
    ...
    if is_first_time:
        ...
        if not config.get("auto_start_prompt_shown", False):
            ...
            desc_label = tk.Label(top_row, text="您是否想要开机自启动本工具？",
                                  font=("微软雅黑", 15, "bold"), bg="#ffffff", fg="#34495e")
            desc_label.pack(side="left")

            # 文本框（紧贴上方，无外边距）
            info_text = tk.Text(main_frame, height=2, wrap="word", font=("微软雅黑", 12),
    ...
    def show_update_prompt():
        ...
        title_label = tk.Label(main_frame, text=f"✨ 发现新版本 v{remote_version}.0！",
                               font=("微软雅黑", 14, "bold"), bg="#ffffff", fg="#2c3e50")
        ...
        # 更新内容文本框（紧贴上方，无外边距）
        notes_text = tk.Text(main_frame, height=4, wrap="word", font=("微软雅黑", 10),
                             bg="#f8f9fa", relief="flat", borderwidth=1)
        ...
        btn_ignore = tk.Button(button_container, text="忽略本次更新并不再提醒", command=on_ignore,
                               bg="#ffffff", fg="#888888", font=("微软雅黑", 9),
...
class WallpaperSidebar:
    def __init__(self, master, folder, current_path, log_path):
        self.master = master
        self.folder = folder
        ...
        try:
            ...
            ctypes.windll.user32.MessageBoxW(0, "无法读取壁纸文件夹", "错误", 0)
            self.master.destroy()
            return
        if not images:
            log_to_file("文件夹中没有图片", log_path)
            ctypes.windll.user32.MessageBoxW(0, "壁纸文件夹中没有图片", "提示喵", 0)
            self.master.destroy()
            return
    ...
    def setup_ui(self):
        ...
        close_btn = tk.Label(self.master, text="✕", font=("Segoe UI", 14),
        ...
        title = tk.Label(self.master, text="📷 壁纸列表", font=("微软雅黑", 12, "bold"),
                         bg='#f0f0f0', fg='#333')
        ...
        def do_highlight_and_scroll():
            ...
            def scroll_to_item():
                ...
                try:
                    ...
                    frame_h = item['frame'].winfo_height()
    ...
    def on_thumbnail_click(self, target_path):
        ...
        try:
            ...
            # 由于已经合并，直接调用主程序的设置函数
            # 但这里需要调用 set_wallpaper 函数，注意全局变量 root 可能不同
            # 简单方式：通过消息发送或直接调用函数
            # 这里直接调用主程序的 set_wallpaper 函数（需确保可访问）
            global root, config
            push_wallpaper(target_path)
            set_wallpaper_direct(target_path, "侧边栏切换")
            log_to_file(f"已切换壁纸", self.log_path)
        except Exception as e:
